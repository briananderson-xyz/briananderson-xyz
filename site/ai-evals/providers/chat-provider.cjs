class ChatProvider {
  constructor(options) {
    ...
    this.timeoutMs = Number(process.env.AI_EVAL_TIMEOUT_MS || this.config.timeoutMs || 20000);
  }
  ...
  async callApi(prompt, context) {
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Chat eval request failed with status ${response.status}: ${responseText.slice(0, 500)}`
          );
        }
    ...
    const detail =
      ...
      lastError?.message ||
      String(lastError);
    throw new Error(`Chat eval transport failed after retries: ${detail}`);
  }
}
