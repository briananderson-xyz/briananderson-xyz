...
We built the first version that semester. A **Ruby on Rails** backend with an **Android** app. It worked well enough that we signed up for the class _again_ to build the **iOS** version in Objective-C.

## What We Built

**Fill In My Blank** let users play with their friends to create hilarious and often inappropriate results through a combination of _Blanks_ and _Fillers_. Real-time multiplayer sessions, cross-platform play between Android and iOS, and custom data synchronization that kept everything in sync.
