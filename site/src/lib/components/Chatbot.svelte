<script lang="ts">
  ...
  import { buildChatRequest } from "$lib/utils/chatRequest";
  ...
  async function handleSendMessage(content: string) {
    ...
    const requestPayload = buildChatRequest(messages, content);

    // Optimistically render the current turn after snapshotting prior history.
    ...
    try {
      // Call the Cloud Run API (through the local SvelteKit proxy in development).
    ...
    >
      {#if messages.length === 0}
        <div class="text-center text-terminal-text/70 py-12">
          ...
          <p class="text-sm">
            Ask me about Brian's experience, skills, projects, or paste a job description for a fit
            analysis.
          </p>
